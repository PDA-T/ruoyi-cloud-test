package com.streaming.play.service.impl;
...
import com.streaming.play.mapper.StreamRtmpAddressMapper;
...
public class StreamRtmpAddressServiceImpl extends ServiceImpl<StreamRtmpAddressMapper, StreamRtmpAddress> implements StreamRtmpAddressService {
	 ...
	 * SRS的推流地址
	 */
	private static final String SRS_PUSH_ADDRESS = "rtmp://43.143.220.95/live/";
	 ...
	 * @throws Exception
	 */
	private static void grabAndPush(String sourceFilePath, String PUSH_ADDRESS, String name) throws Exception {
		// ffmepg日志级别
		...
		FFmpegFrameRecorder recorder = new FFmpegFrameRecorder(PUSH_ADDRESS + name,
				frameWidth,
				frameHeight,
				audioChannels);
	...
	public StreamRtmpAddress play(StreamRtmpAddress rtmp) {
		...
		new Thread(() -> {
			try {
				grabAndPush(rtmp.getUrl(), SRS_PUSH_ADDRESS, String.valueOf(i));
			} catch (Exception e) {
				throw new ServiceException("无视频");
			}
		}).start();
		rtmp.setName(String.valueOf(i));
		return rtmp;
	}
}
